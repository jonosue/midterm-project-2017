...
function eventExists(urlCheck, cb) {
  knex('events')
  .where('events.short_url', urlCheck)
  .count('id')
  .asCallback(function (err, result) {
    const responseValue = Number(result[0].count);
    cb(responseValue);
...
// This route handles the creation of an event, based on the initial details that the user provides through the form on the index page.
...
// This route handles how to access the second part of the event creation process, where you choose dates.
    ...
    if (req.session.admin_id == cv) {
      findEventID(req.session.admin_id, function(id) {
        ...
        .asCallback(function (err, result) {
          ...
          .select('events_responses.user_id', 'users.email', 'events_responses.eventsdates_id', 'events_responses.id',
            'users.first_name', 'users.last_name', 'events_dates.datetime', 'events_responses.response', 'events.name',
            'events.location', 'events.description')
...
// This route handles how individual dates are added to the database for the event which is being created.
...
// This route handles how the event is finalized once all of the dates are selected.
...
// This route handles how all users get to the page where they can vote on what dates they want to pick for an event.
app.get("/:shortURL", (req, res) => {
  eventExists(req.params.shortURL, function(count) {
    if (count) {
      knex.from('events')
      .orderByRaw('events_responses.user_id asc, events_dates.id asc')
      .where('events.short_url', req.params.shortURL)
      ...
      .leftJoin('events_responses', 'events_dates.id', 'events_responses.eventsdates_id')
      .leftJoin('users', 'events_responses.user_id', 'users.id')
      .select('events_responses.user_id', 'users.email', 'events.short_url', 'events_responses.eventsdates_id',
        'events_responses.id', 'users.first_name', 'users.last_name', 'events_dates.datetime',
        'events_responses.response', 'events.name', 'events.location', 'events.description')
      .asCallback(function (err, userResponses) {
...
// This route handler is the most complex one, because it handles the populating, updating, and retrieving of all of the user responses for an event.
